...
export default function OwnerComplaintsPage() {
  ...
  useEffect(() => {
    if (token && user?.role === "OWNER") fetchComplaints();
  ...
  const filtered = complaints.filter((c) => {
    ...
    return c.title.toLowerCase().includes(q) || c.complainant.name.toLowerCase().includes(q) || c.booking?.room.title.toLowerCase().includes(q);
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-10 text-center dark:border-gray-700 dark:bg-gray-900">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">You have no complaints filed against you</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((c) => (
              <Link key={c.id} href={`/dashboard/owner/complaints/${c.id}`} className="block rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      From: {c.complainant.name} · {c.booking ? `Room: ${c.booking.room.title}` : "Platform"}
                    </p>
