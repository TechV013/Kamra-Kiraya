...
import { AlertTriangle, Search, ChevronLeft, ChevronRight, Eye } from "lucide-react";
...
export default function AdminComplaintsPage() {
  ...
  useEffect(() => {
    if (token && user?.role === "ADMIN") fetchComplaints();
  ...
  const filtered = complaints.filter((c) => {
    ...
    return c.title.toLowerCase().includes(q) || c.complainant.name.toLowerCase().includes(q) || c.respondent.name.toLowerCase().includes(q) || c.booking?.room.title.toLowerCase().includes(q);
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Manage all complaints on the platform</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by title, name, or room..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm outline-none focus:border-maroon-500 focus:ring-1 focus:ring-maroon-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
          </div>
        ...
        {loading ? (
          <div className="space-y-3">{[1, 2, 3, 4].map((i) => <div key={i} className="h-24 bg-gray-200 dark:bg-gray-800 rounded-2xl animate-pulse" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-10 text-center dark:border-gray-700 dark:bg-gray-900">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">All complaints will appear here</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((c) => (
              <Link key={c.id} href={`/dashboard/admin/complaints/${c.id}`} className="block rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {c.complainant.name} → {c.respondent.name} · {c.booking ? `Room: ${c.booking.room.title}` : "Platform"}
                    </p>
